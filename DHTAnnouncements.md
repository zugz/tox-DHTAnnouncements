...
On receiving a Route Request packet, we check that the addressee port is in 
the range 33445-33454, and if we send a Route Deliver packet to the addressee 
IP/Port with sendback set to a random 24-byte nonce followed by the addressee 
IP/Port, authenticated and encrypted using the nonce and the same symmetric 
key used to generate Ping IDs above.

The restriction on ports is to prevent any abuse which could be enabled by 
having attackers able to cause DHT nodes to send largely attacker-determined 
packets to arbitrary ports where they might be interpreted under other 
protocols.

Using an encrypted sendback prevents such abuse on the return path.
...
On receiving a Route Deliver packet, we attempt to handle the data as a DHT 
packet containing one of the request packets associated with the announcement 
system defined above. If a response is generated, it should be wrapped in a 
Route Reply packet, with the sendback received in the current packet.
...
On receiving a Route Reply packet, we authenticate and decrypt the sendback to 
obtain an IP/Port or information identifying one of our TCP clients, and if 
this succeeds we send a (TCP) Route Response packet to that IP/Port containing 
the source IP/Port of the current packet and the data.
...
On receiving a Route Response packet, we attempt to interpret the data as a 
DHT packet containing one of the response packets associated with the 
announcement system defined above.
...
sendback should identify the client to the server (as is currently done for 
TCP onion packets).
